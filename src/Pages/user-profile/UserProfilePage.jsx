...
const isArtworksLoading = useArtworksStore(state => state.isLoading);  const navigate = useNavigate();

  // Fetch artworks for the current user
  useEffect(() => {
    if (user?.id) {
      fetchArtworks(user.id); // Fetch only the current user's artworks
    }
  }, [user, fetchArtworks]);
